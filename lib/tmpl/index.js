...
function getContext(module, extContext) {
    ...
    if (!BaseContext) {
        ...
        typeMap[tmpl.TYPE_ATTRIBUTE_CLASS] = ATTRIBUTE;
        typeMap[tmpl.TYPE_ATTRIBUTE_STYLE] = ATTRIBUTE;
        typeMap[tmpl.TYPE_ATTRIBUTE_EVENT] = ATTRIBUTE;
        typeMap[tmpl.TYPE_TEXT] = TEXT;
        typeMap[tmpl.TYPE_COMMENT] = COMMENT;

        // try to use walker from basis.js
        if (tmpl.declaration && typeof tmpl.declaration.walk === 'function') {
            walker = function(ast, handlers) {
                tmpl.declaration.walk(ast, function(type, node, parent) {
                    var handler = handlers[this.tokenType(node)];
        ...
        } else { // own implementation of walker (old basis.js has no AST walker)
            walker = function(ast, handlers, parentToken, stIdx) {
        ...
        BaseContext.prototype = {
            ...
            getChildren: function(token) {
                switch (token[tmpl.TOKEN_TYPE]) {
                    ...
                    case tmpl.TYPE_CONTENT:
                        return token.slice(tmpl.CONTENT_CHILDREN);

                    default:
                        throw new Error('[ERROR] unsupported token type for getChildren(): ' + token[tmpl.TOKEN_TYPE]);
                }
            },
            ...
            tokenName: function(token) {
                switch (token[tmpl.TOKEN_TYPE]) {
                    ...
                    case tmpl.TYPE_ATTRIBUTE_CLASS:
                        return 'class';

                    case tmpl.TYPE_ATTRIBUTE_STYLE:
                        return 'style';

                    case tmpl.TYPE_ATTRIBUTE_EVENT:
