...
var fnWalker = function(token, scope){
  ...
  if (name)
    scope.put(name, token[0], token, token.start);

  var newScope = new Scope('function', scope);
  newScope.sourceToken = token;
  this.scopes.push(newScope);

  newScope.put('arguments', 'arguments', null, null, args);
  for (var i = 0; i < args.length; i++)
    newScope.put(args[i], 'arg', null, args.loc && args.loc[i], { list: args, index: i });
...
var varWalker = function(token, scope){
  ...
  for (var i = 0, def; def = defs[i]; i++)
  {
    ...
    scope.put(name, 'var', val, def.start);
  }
};
...
function process(ast, scope){
  ...
  return walker.walk(ast, {
    ...
    'try': function(token, scope){
      if (token[2])
      {
        var scope = new Scope('catch', scope);
