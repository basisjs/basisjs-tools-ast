  ...
  return walker.walk(ast, {
    CallExpression: function(token, scope){
      ...
      if (fn)
      {
        ...
        if (fn.type === 'FunctionExpression' || fn.type === 'FunctionDeclaration' || fn.type === 'ArrowFunctionExpression')
        {
          if (fn.run)
          {
            if (verbose)
            ...
            fn.run.call(
              this,
              token,
              expr.type === 'Identifier' || expr.type === 'MemberExpression' ? scope.resolve(expr.object || expr.name) : null,
              args,
              scope
            );
