...
function process(ast, context) {
    ...
    return walker.walk(ast, {
                ...
                if (['FunctionExpression', 'FunctionDeclaration', 'ArrowFunctionExpression'].indexOf(fn.type) > -1) {
                    if (fn.run) {
                        ...
                        fn.run.call(
                            this,
                            token,
                            ['Identifier', 'MemberExpression'].indexOf(expr.type) > -1 ? scope.resolve(expr) : null,
                            args,
                            scope
                        );
        ...
        Identifier: function(token, scope) {
            ...
            var name = token.namespaces;
            var ref;

            // TODO: make correct test for global
            if (scope.isGlobal(name) && name != 'global' && name != 'this') {
                ref = scope.get(name);
        ...
        ObjectExpression: function(token, scope) {
            ...
            for (var i = 0, prop; prop = props[i]; i++) {
                obj[prop.key.name] = scope.resolve(prop.value) || prop.value;
            }
        ...
        AssignmentExpression: function(token, scope) {
            ...
            if (op == '=') {
                ...
                if (lvalue.type == 'MemberExpression') {
                    ...
                    if (dest && dest.obj) {
                        dest.obj[lvalue.property] = scope.resolve(rvalue) || rvalue;
                    }
